...
def validate_asset_repair_permissions(doc, method=None):
    """
    Security validation for Asset Repair documents
    - Once submitted, only approval_status can be manually changed
    - approval_time and approved_by are set server-side automatically
    - Prevents any user (including managers) from editing repair details after submission

    Hook this via: doc_events in hooks.py
    """
    if doc.docstatus == 1:  # Document is submitted
        # Get original document from database (bypass cache)
        old_doc = frappe.db.get_value("Asset Repair", doc.name, "*", as_dict=True)
        if not old_doc:
            return

        # List of ALL fields that CANNOT be changed after submission
        protected_fields = [
        ...
        for field in protected_fields:
            ...
            # Convert to strings for comparison to handle None values
            if str(old_value or '') != str(new_value or ''):
                log_security_event("UNAUTHORIZED_FIELD_MODIFICATION", {
                    "repair": doc.name,
                    "field": field,
        ...
        # PROTECTED: Users cannot manually set approval_time or approved_by
        if doc.get('approval_time') != old_doc.get('approval_time'):
            if not doc.flags.auto_approval:  # Only allow if set programmatically
                frappe.throw(
        ...
        if doc.get('approval_status') != old_doc.get('approval_status'):
            if doc.get('approval_status') in ['Approved', 'Rejected']:
                # Set approval fields automatically with server data
                doc.flags.auto_approval = True
...
def generate_maintenance_qr(asset_name):
    """
    Generate QR code for maintenance portal
    Returns QR code as base64 data URI

    Args:
        asset_name: Asset ID (e.g., ACC-ASS-2025-00170)

    Returns:
        dict: {
            "success": True,
            "data_uri": "data:image/png;base64,...",
            "url": "https://domain/maintenance?asset=...",
            "asset_name": "..."
        }
    """
    check_rate_limit("generate_qr", max_requests=50, window_seconds=60)

    # Verify asset exists
    if not frappe.db.exists("Asset", asset_name):
        frappe.throw(_("Asset {0} not found").format(asset_name))

    # Get clean site URL (without port)
    site_config = frappe.get_site_config()
    site_url = site_config.get("host_name", frappe.utils.get_url())

    # Remove port if present (for development environments)
    if ":8000" in site_url:
        site_url = site_url.replace(":8000", "")

    # Generate maintenance portal URL
    maintenance_url = f"{site_url}/maintenance?asset={asset_name}"
    ...
    try:
        ...
        frappe.throw(_("QR code library not installed. Please run: pip install qrcode[pil]"))
    except Exception as e:
